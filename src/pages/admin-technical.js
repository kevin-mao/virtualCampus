...
import { AdminInterviewModal, Template, Title, } from "../components";
...
class AdminTechnical extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      open: false,
  ...
  async getEvents() {
    ...
    if(approvedEvents){
        ...
        approvedEventsMap = approvedEvents.docs.map(doc => {
            const obj = this.convertEventsTime(doc.data()); 
            obj.event_id = doc.id;
            return obj 
        });
    }
  ...
  render() {
    return (
      <Template active={"technical"} title={"Technical"}>
        ...
        {this.state.open && 
          <AdminInterviewModal open={this.state.open} closeDo={this.closeDo} event={this.state.event} setSubmitStatus={this.setSubmitStatus}/>}
      </Template>
    );
  }
}

export default withStyles(useStyles)(AdminTechnical);
